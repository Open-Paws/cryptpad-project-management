...
var resolveCandidatePath = function (candidate) {
    if (typeof(candidate) !== 'string') { return null; }
    var resolvedCandidate = Path.resolve(candidate);
    var cursor = resolvedCandidate;
    while (!Fs.existsSync(cursor)) {
        var parent = Path.dirname(cursor);
        if (parent === cursor) { return null; }
        cursor = parent;
    }
    var realCursor = realpath(cursor);
    if (!realCursor) { return null; }
    var relative = Path.relative(cursor, resolvedCandidate);
    return Path.resolve(realCursor, relative);
};
